...
Puppet::Type.newtype(:certificate_trust) do
  @doc = "Manage certificate trust settings."
  
  # TODO: ensurable seems to do ABSOLUTELY NOTHING here. Could be a syntax error in the type?
  ensurable
  # present -> trusted
  # absent -> remove trust
    
  newparam(:cn) do
    desc "The CN (Common Name) of the certificate"
    isnamevar
  end
  
  newparam(:policy_string) do
    desc "The policy string (normally the hostname)"
  end
